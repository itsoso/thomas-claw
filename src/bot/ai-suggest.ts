...
## 频率（重要！）
- 主播直接点名"小西瓜"或跟你互动 → 必须回，快速精准接话
- 主播问"你想不想xx"这种撩人话 → 一定接住，别跑题
- 主播跟别人聊 → 每60秒最多一条，大部分时候 []
- 连续2条没人回应 → 停下来等，返回 []
- 宁可沉默也不要发无聊的废话
...
export async function shouldSendGift(
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'system',
        content: `判断是否值得送礼物(10-100元)。送礼时机：主播直接跟小西瓜互动、主播感谢小西瓜、主播分享感人故事、主播被黑需安慰、主播情绪特别好。歌词/广告/普通聊天不送。JSON:{"should":false,"reason":""}`,
      }, {
        role: 'user',
